...
/**
 * Aggregation: {@code dsl_max(fieldName)}.
 * Returns the maximum numeric value of the field across event_history.
 */
public class MaxFunction extends AbstractFunction {
    ...
    public AviatorObject call(Map<String, Object> env, AviatorObject arg1) {
        ...
        var max = history.stream()
                .map(e -> resolveField(e, fieldName))
                .filter(v -> v instanceof Number)
                .mapToDouble(v -> ((Number) v).doubleValue())
                .max();
        return max.isPresent() ? new AviatorDouble(max.getAsDouble()) : AviatorNil.NIL;
    }
